      ...
      <div class="bg-surface border border-outline/30 rounded-3xl p-8 shadow-sm w-full max-w-[400px] mx-auto relative z-10">
        ...
        <div class="text-center mb-8">
            <div class="w-16 h-16 bg-primary-container text-primary rounded-2xl flex items-center justify-center mx-auto mb-4">
                <app-icon name="school" [isFilled]="true" class="text-[32px] !w-[32px] !h-[32px]"></app-icon>
            </div>
            <h1 class="text-[26px] font-bold text-on-surface tracking-tight mb-2">Bem-vindo de volta</h1>
            <p class="text-[15px] text-on-surface-variant">Faça login para continuar seus estudos</p>
        </div>
        ...
        <form (ngSubmit)="onSubmit()" #loginForm="ngForm" class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-1.5">
            <div class="flex items-center justify-between ml-1">
              <label class="text-[14px] font-bold text-on-surface">Senha</label>
              <a routerLink="/auth/esqueci-a-senha" class="text-[13px] font-bold text-primary hover:underline active:opacity-70">Esqueceu?</a>
            </div>
            <div class="relative">
          ...
          <div class="mt-4">
            <app-primary-button type="submit" [disabled]="loading() || !loginForm.valid">
              @if(loading()) {
                Entrando...
              } @else {
                Entrar
        ...
        <p class="text-center text-[14px] text-on-surface-variant mt-8">
          Ainda não tem conta? 
          <a routerLink="/auth/cadastro" class="font-bold text-primary hover:underline active:opacity-70 ml-1">Criar agora</a>
        </p>
...
export class LoginComponent {
  ...
  async onSubmit() {
    ...
    try {
      ...
      console.error(error);
      this.notification.error('E-mail ou senha incorretos.');
    } finally {
